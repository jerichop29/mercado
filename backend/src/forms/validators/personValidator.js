class PersonValidator {
    static validatePersonData(data) {
        ...
        if (!data.FName || typeof data.FName !== 'string' || !data.FName.trim() || !/^([^a-zA-Z])$/.test(data.FName)) {
            errors.push('First name (FName) is required and must be a non-empty string.');
        }

        if (!data.LName || typeof data.LName !== 'string' || !data.LName.trim()|| !/^([^a-zA-Z])$/.test(data.LName)) {
            errors.push('Last name (LName) is required and must be a non-empty string.');
        }

        if (data.MName && (typeof data.MName !== 'string' || !data.MName.trim())|| !/^([^a-zA-Z])$/.test(data.MName)) {
